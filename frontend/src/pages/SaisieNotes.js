...
const SaisieNotes = () => {
  ...
  const handleNoteChange = (eleveId, matiere, valeur) => {
    setNotes(prev => {
      const noteEleve = prev[eleveId] || { etude_texte: 0, aem: 0, dictee: 0, math: 0 };
      return {
        ...prev,
        [eleveId]: {
          ...noteEleve,
  ...
  const handleEnregistrer = async (eleveId) => {
    // Récupérer les notes actuelles ou notes existantes
    const noteData = notes[eleveId] || { etude_texte: 0, aem: 0, dictee: 0, math: 0 };
    ...
    // Validation minimums (notes négatives)
    if (noteData.etude_texte < 0 || noteData.aem < 0 || noteData.dictee < 0 || noteData.math < 0) {
      toast.error('Les notes ne peuvent pas être négatives');
      return;
    }
    ...
    try {
      const data = {
        ...
        eleve_id: eleveId,
        etude_texte: noteData.etude_texte || 0,
        aem: noteData.aem || 0,
        dictee: noteData.dictee || 0,
        math: noteData.math || 0
      };

      if (notesExistantes[eleveId]) {
        // Mise à jour de notes existantes
        await axios.put(`${API}/notes/${notesExistantes[eleveId]}`, {
          etude_texte: data.etude_texte,
          aem: data.aem,
          dictee: data.dictee,
          math: data.math
        });
  ...
  return (
    <div className="main-container">
      ...
      <div style={{ 
        ...
        <strong>ℹ️ Information :</strong> Vous pouvez saisir les notes partiellement (laisser des champs vides si nécessaire). 
        Les élèves avec notes enregistrées apparaissent sur fond bleu clair avec le badge "Enregistré". 
        Vous pouvez modifier les notes à tout moment et cliquer sur "Enregistrer" pour mettre à jour.
      </div>
              ...
              <Table data-testid="table-notes">
                ...
                <TableBody>
                  {eleves.map((eleve) => {
                    ...
                    const hasExistingNotes = notesExistantes[eleve.id];
                    return (
                      <TableRow 
                        key={eleve.id} 
                        data-testid={`note-row-${eleve.id}`}
                        style={{ background: hasExistingNotes ? '#f0f9ff' : 'white' }}
                      >
