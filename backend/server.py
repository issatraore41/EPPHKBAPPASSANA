...
class Eleve(BaseModel):
    ...
    sexe: str = "M"  # "M" pour Masculin/Garçon, "F" pour Féminin/Fille
...
async def calculer_classement(composition_id: str):
    ...
    # Séparer les élèves présents (total > 0) et absents (total = 0)
    notes_presentes = [n for n in notes if n['total'] > 0]
    notes_absentes = [n for n in notes if n['total'] == 0]
    
    # Trier les notes présentes par total décroissant
    notes_triees = sorted(notes_presentes, key=lambda x: x['total'], reverse=True)
    
    # Mettre à jour les rangs pour les élèves présents
    ...
    # Mettre rang à 0 pour les absents (pour les identifier facilement)
...
async def lister_eleves(classe_id: Optional[str] = None):
    ...
    eleves_tries = sorted(eleves, key=lambda x: (x['nom'].upper(), x['prenom'].upper()))
    return eleves_tries
...
async def creer_note(note_input: NoteCreate):
    # Vérifier si une note existe déjà pour cet élève dans cette composition
    note_existante = await db.notes.find_one({
        "composition_id": note_input.composition_id,
        "eleve_id": note_input.eleve_id
    }, {"_id": 0})
    
    if note_existante:
        # Si la note existe déjà, la mettre à jour au lieu de créer un doublon
        ...
        await db.notes.update_one(
            ...
            {"$set": {
                "etude_texte": note_input.etude_texte,
                "aem": note_input.aem,
                "dictee": note_input.dictee,
                "math": note_input.math,
                "total": total,
                "moyenne": moyenne,
        ...
        note_maj = await db.notes.find_one({"id": note_existante['id']}, {"_id": 0})
        return note_maj
...
async def obtenir_statistiques(composition_id: str):
    ...
    eleve_ids = [n['eleve_id'] for n in notes]
    eleves = await db.eleves.find({"id": {"$in": eleve_ids}}, {"_id": 0}).to_list(1000)
    eleves_map = {e['id']: e for e in eleves}
    
    effectif = len(notes)
    # Compter les absents (total = 0)
    absents = sum(1 for n in notes if n['total'] == 0)
    presents = effectif - absents
    # Ne compter que les élèves présents pour les admis
    admis = sum(1 for n in notes if n['total'] > 0 and n['moyenne'] >= 5.0)
    pourcentage_reussite = round((admis / presents * 100), 2) if presents > 0 else 0
    ...
    for note in notes:
        if note['total'] > 0:  # Uniquement les présents
            eleve = eleves_map.get(note['eleve_id'])
            if eleve:
    ...
    garcons_admis = sum(1 for n in garcons if n['moyenne'] >= 5.0)
    garcons_refuses = nb_garcons - garcons_admis
    filles_admises = sum(1 for n in filles if n['moyenne'] >= 5.0)
    filles_refusees = nb_filles - filles_admises
    ...
    return {
        ...
        "nb_filles": nb_filles,
        "garcons_admis": garcons_admis,
        "garcons_refuses": garcons_refuses,
        "filles_admises": filles_admises,
        "filles_refusees": filles_refusees
    }

# ========== SUIVI SUR 8 MOIS ==========

@api_router.delete("/notes/doublons/{composition_id}")
async def nettoyer_doublons(composition_id: str):
    """Supprime les notes en double pour une composition"""
    notes = await db.notes.find({"composition_id": composition_id}, {"_id": 0}).to_list(10000)
    
    # Grouper par eleve_id
    notes_par_eleve = {}
    for note in notes:
        eleve_id = note['eleve_id']
        if eleve_id not in notes_par_eleve:
            notes_par_eleve[eleve_id] = []
        notes_par_eleve[eleve_id].append(note)
    
    # Supprimer les doublons (garder la première, supprimer les autres)
    notes_supprimees = 0
    for eleve_id, eleve_notes in notes_par_eleve.items():
        if len(eleve_notes) > 1:
            # Garder la première note, supprimer les autres
            for note in eleve_notes[1:]:
                await db.notes.delete_one({"id": note['id']})
                notes_supprimees += 1
    ...
    return {
        "message": f"{notes_supprimees} notes en double supprimées",
